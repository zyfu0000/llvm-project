...
public:
    ...
    virtual void run(const MatchFinder::MatchResult &Result) {
        ...
        const FunctionDecl *functionDecl                = Result.Nodes.getNodeAs<FunctionDecl>("functionDecl");
        
        if (isUserSourceDecl(interfaceDecl))
            handleInterfaceDecl(interfaceDecl);
        if (isUserSourceDecl(implDecl))
            handleImplementationDecl(implDecl);
        if (isUserSourceDecl(categoryDecl))
            handleCategoryDecl(categoryDecl);
        if (isUserSourceDecl(categoryImplDecl))
            handleCategoryImplDecl(categoryImplDecl);
        if (isUserSourceDecl(explicitCastExpr))
            handleExplicitCastExpr(explicitCastExpr);
        if (isUserSourceDecl(varDecl))
            handleVarDecl(varDecl);
        if (isUserSourceDecl(objcIvarDecl))
            handleObjcIVarDecl(objcIvarDecl);
        if (isUserSourceDecl(objcPropertyDecl))
            handleObjcPropertyDecl(objcPropertyDecl);
        if (isUserSourceDecl(methodDecl))
            handleMethodDecl(methodDecl);
        if (isUserSourceDecl(messageExpr))
            handleMessageExpr(messageExpr);
        if (stringLiteral)
            handleStringLiteral(stringLiteral);
        if (isUserSourceDecl(typedefDecl))
            handleTypedefDecl(typedefDecl);
        if (isUserSourceDecl(functionDecl))
            handleFunctionDecl(functionDecl);
    }
    ...
    void handleMessageExpr(const ObjCMessageExpr *messageExpr){
        OPCheckContextInvalid(messageExpr)
    ...
    void handleStringLiteral(const clang::StringLiteral *stringLiteral){
        ...
        if (isNeedObfuscateClassName(content.str())) {
            SourceLocation loc = stringLiteral->getBeginLoc();//loc的位置在
            string newClassName = getNewClassName(content.str());
    ...
    void handleFunctionDecl(const FunctionDecl *functionDecl) {
        if (functionDecl->hasAttr<AnnotateAttr>()) {
            AnnotateAttr *attr = functionDecl->getAttr<AnnotateAttr>();
    ...
    void handleMethodDecl(const ObjCMethodDecl *methodDecl){
        OPCheckContextInvalid(methodDecl)
...
public:
    ObfASTConsumer(Rewriter &aRewriter,CompilerInstance *aCI) :handlerMatchCallback(aRewriter,aCI)  {
        ...
        //实例方法
//        matcher.addMatcher(objcMessageExpr(isInstanceMessage()).bind("objcMessageExpr"),   &handlerMatchCallback);
        //显式转换表达式
        ...
        matcher.addMatcher(functionDecl().bind("functionDecl"),   &handlerMatchCallback);
